...
# jinja context. Every os.environ value is a string, and main.cf asks
# "{{ 'dane' if DEFER_ON_TLS_ERROR else 'may' }}" -- "False" is a non-empty
# string and therefore true, so the setting could not be turned off. set_env()
# has already coerced it the way the admin's ConfigManager does; take the
# boolean from there. The rest stays as strings on purpose: the whole coerced
# mapping would turn REJECT_UNLISTED_RECIPIENT=yes into the literal True in
# master.cf, which is not a Postfix boolean.
