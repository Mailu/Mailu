...
def test_auth_header_variants(app, client, create_user_and_token, grant_domain_access):
    with app.app_context():
        ...
        headers = {'authentication': f'u@example.com:{TOKEN}'}
        rv = client.post('/api/alias/random/new', headers=headers, json={})
        assert rv.status_code == 201

        # Missing colon -> should be rejected as invalid token format (401)
        headers = {'Authentication': f'u@example.com{TOKEN}'}
        rv = client.post('/api/alias/random/new', headers=headers, json={})
        assert rv.status_code == 401

        # Wrong format (no email) -> rejected
        ...
        # Authentication header with email:token format works
        ...
        headers = {'Authentication': f'bw@example.com:{"B" * 32}'}
